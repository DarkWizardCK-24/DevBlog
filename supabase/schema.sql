...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.blog_profiles (id, username, display_name, avatar_url)
  VALUES (
    NEW.id,
...
CREATE POLICY "blog_profiles_public_read"  ON blog_profiles FOR SELECT USING (true);
CREATE POLICY "blog_profiles_owner_insert" ON blog_profiles FOR INSERT WITH CHECK (auth.uid() = id);
CREATE POLICY "blog_profiles_owner_update" ON blog_profiles FOR UPDATE USING (auth.uid() = id);

-- Posts: public read if published, owner full access
...
-- Comments: readable on public posts, owner write
CREATE POLICY "blog_comments_public_read"  ON blog_comments FOR SELECT
  USING (EXISTS (SELECT 1 FROM blog_posts WHERE id = post_id AND (published = true OR user_id = auth.uid())));
